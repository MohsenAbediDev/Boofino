          ...
          <div className='dashboard-container h-fit py-5 flex flex-col gap-y-9 justify-center'>
            {
              datas && (<>
                ...
                <div className="p-5 bg-primary rounded-lg flex w-fit mx-auto">
                  <img src={selectedPic ? filePath : datas.imgUrl} alt="" className="w-40 h-40 rounded-lg" />
                ...
                <div className='flex justify-end w-4/5 gap-x-5'>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (imgUrl || (name && name !== datas.name)
                        || (+price && +price !== +datas.price)
                        || (group && group !== datas.group)
                        || (+off && +off == 0 || +off !== +datas.off)
                        || (+count && +count !== +datas.itemCount)) {

                        try {
                          const res = await fetch(`http://localhost:3000/editproduct/${params.name}`, {
                            method: 'PUT',
                            credentials: 'include',
                            headers: {
                            ...
                            body: JSON.stringify({
                              ...
                              finalPrice: +Math.floor((price || datas.price) - ((off || datas.off) * (price || datas.price)) / 100),
                              group: group || datas.group,
                              off: off,
